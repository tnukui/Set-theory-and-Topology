...
密着位相は一般に距離化可能でない。実際、密着位相$\mathcal{O}=\{X,\emptyset\}$が距離化可能であるためには、ある距離関数$d$が存在して、任意の$x\in X,\ \epsilon>0$に対して、$N(x;\epsilon)$が、
\[N(x;\epsilon)\in\mathcal{O}\]
となる必要があるが、$N(x;\epsilon)$は一般に$X$とも$\emptyset$とも一致しない。
